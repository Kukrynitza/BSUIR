...
def save_profiles(profiles: dict[str, dict]) -> None:
    ...
    with get_connection() as conn:
        ...
        data = []
        for lang, methods in profiles.items():
            for method, lexemes in methods.items():
                for lexeme, freq in lexemes.items():
                    data.append((lang, lexeme, freq, method))
        if not data:
            cur.close()
            return
        psycopg2.extras.execute_values(
            cur,
            "INSERT INTO language_profiles (language, lexeme, frequency, method) VALUES %s",
            data,
            template="(%s, %s, %s, %s)",
        )
        cur.close()