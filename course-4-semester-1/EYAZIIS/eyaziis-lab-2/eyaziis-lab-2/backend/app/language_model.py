...
class ShortWordsRecognizer:
    ...
    def recognize(self, text: str, profiles: dict[str, dict[str, float]]) -> dict:
        ...
        return {
            "language": best,
            "confidence": confidence.get(best, 0.0),
            "score": details[best]["log_prob"],
            "oop": details[best]["oop"],
            "matched": details[best]["matched"],
            "distances": details,
            "method": "short_words",
        }
...
class FrequentWordsRecognizer:
    ...
    def recognize(self, text: str, profiles: dict[str, dict[str, float]]) -> dict:
        ...
        compared = [lemma for lemma in doc_all if lemma in vocab] or doc_all
        if not compared:
            return {
                "language": "unknown",
                "confidence": 0.0,
...
class NeuralNetworkRecognizer:
    ...
    def recognize(self, text: str) -> dict:
        if not self._trained or self.clf is None or self.label_encoder is None:
            return {"language": "не обучена", "confidence": 0.0, "method": "neural_network"}
