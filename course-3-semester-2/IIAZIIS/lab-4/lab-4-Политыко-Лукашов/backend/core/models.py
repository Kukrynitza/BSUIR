...
class Sentence(Base):
    ...
    semantic_annotations = relationship(
        'SemanticAnnotation',
        back_populates='sentence',
        cascade='all, delete-orphan',
        order_by='SemanticAnnotation.id',
    )
    semantic_links = relationship(
        'SemanticLink',
        back_populates='sentence',
        cascade='all, delete-orphan',
        order_by='SemanticLink.id',
    )
    ...
    def to_dict(self):
        return {
            ...
            'semantic_annotations': [
                a.to_dict() for a in (self.semantic_annotations or [])
            ],
            'semantic_links': [l.to_dict() for l in (self.semantic_links or [])],
        }
...
class Token(Base):
    ...
    semantic_annotations = relationship(
        'SemanticAnnotation', back_populates='token', cascade='all, delete-orphan'
    )
...
class SemanticAnnotation(Base):
    ...
    entity_category = Column(String(100), default='')
    semantic_role = Column(String(100), default='')
    semantic_role_description = Column(String(500), default='')
    syntax_basis = Column(String(200), default='')
    concept_label = Column(String(500), default='')
    concept_uri = Column(String(500), default='')
    knowledge_source = Column(String(50), default='rules')
    created_at = Column(DateTime, default=datetime.now)

    sentence = relationship('Sentence', back_populates='semantic_annotations')
    token = relationship('Token', back_populates='semantic_annotations')
    ...
    def to_dict(self):
        return {
            ...
            'token_text': self.token.token_text if self.token else '',
            'entity_category': self.entity_category,
            'semantic_role': self.semantic_role,
            'semantic_role_description': self.semantic_role_description,
            'syntax_basis': self.syntax_basis,
...
class SemanticLink(Base):
    ...
    link_label_ru = Column(String(300), default='')
    knowledge_source = Column(String(50), default='syntax_projection')
    created_at = Column(DateTime, default=datetime.now)

    sentence = relationship('Sentence', back_populates='semantic_links')
    ...
    def to_dict(self):
        return {
            ...
            'link_label_ru': self.link_label_ru,
            'knowledge_source': self.knowledge_source,
        }
