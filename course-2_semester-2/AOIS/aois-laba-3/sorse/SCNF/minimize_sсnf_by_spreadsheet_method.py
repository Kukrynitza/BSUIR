...
def get_maxterms(expression: str):
    variables = parse_expression(expression)
    truth_table = generate_truth_table(variables)
    maxterms = [values for values in truth_table if not evaluate_expression(expression, variables, values)]
    return variables, maxterms
...
def minimize_sknf(expression: str):
    variables, maxterms = get_maxterms(expression)
    minimized = minimize_karnaugh(maxterms, variables)

    def term_to_sknf(term):
        return "(" + " | ".join(
            f"{variables[i]}" if term[i] == 0 else f"!{variables[i]}" for i in range(len(term)) if term[i] != '-') + ")"

    return " & ".join(term_to_sknf(term) for term in minimized)


def minimize_scnf_by_spreadsheet_method(expr):
    variables, maxterms = get_maxterms(expr)
    print_karnaugh_map(maxterms, variables)
    result = minimize_sknf(expr)
    print(f"Минимизированная форма (СКНФ) табличным методом: {result}")
    return minimize_sknf_by_calculation_method(result)
