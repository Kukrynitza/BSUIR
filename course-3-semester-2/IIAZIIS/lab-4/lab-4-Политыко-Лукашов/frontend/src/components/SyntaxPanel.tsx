...
import { api, Document, AnalysisResult, Sentence, Token, RelationsResult, DependencyTree, ConstituencyTree, DirectSemanticSentence } from '../api/client';
...
function SyntaxPanel() {
  ...
  const [directSemanticBlocks, setDirectSemanticBlocks] = useState<DirectSemanticSentence[] | null>(null);
  ...
  const handleExportSemantic = async (docId: string, docTitle: string) => {
    try {
      const data = await api.exportSemanticJson(docId);
      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${docTitle}_semantic.json`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      setError('Не удалось экспортировать семантику');
    }
  };
  ...
  const handleDirectAnalyze = async () => {
    ...
    try {
      ...
      setDirectSemanticBlocks(treeResult.semantic_analysis ?? []);
    } catch (err) {
  ...
  const isLoading = loading || uploading || analyzing || directAnalyzing || savingDoc || loadingTrees;
  const getLoadingMessage = () => {
    if (uploading) return 'Загрузка файла...';
    if (analyzing) return 'Анализ документа...';
    if (directAnalyzing) return 'Анализ текста...';
    if (savingDoc) return 'Сохранение...';
    if (loadingTrees) return 'Построение деревьев...';
          ...
          <div className="stat-item">
            ...
            <ul>
              <li><span>Предложений:</span> <strong>{stats.total_sentences}</strong></li>
               <li><span>Токенов:</span> <strong>{stats.total_tokens}</strong></li>
  ...
  return (
    <>
      <LoadingOverlay visible={isLoading} message={getLoadingMessage()} />
      <div className="panel">
      ...
      {directAnalysis && (
        <>
          ...
          <SemanticPanel directSemantic={directSemanticBlocks} />
        </>
      )}
      ...
      <div className="upload-section" style={{ marginTop: '2rem' }}>
        ...
        <span className="hint">(TXT, RTF, DOCX)</span>
      </div>
              ...
              {documents.map((doc) => (
                <li key={doc.id} className="document-item">
                  ...
                  <div className="doc-actions">
                    ...
                    <button onClick={() => handleExportSemantic(doc.id, doc.title)}>
                      Семантика
                    </button>
      ...
      {analysis && (
        <div style={{ marginTop: '2rem' }}>
          ...
          <SemanticPanel
            sentences={analysis.analysis}
            docId={analysis.document_id}
            onAnalysisRefresh={(rows) => {
              setAnalysis((prev) => (prev ? { ...prev, analysis: rows } : null));
            }}
          />
        </div>
      )}
