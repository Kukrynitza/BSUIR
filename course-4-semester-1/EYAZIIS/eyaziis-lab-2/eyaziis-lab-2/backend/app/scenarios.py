...
def main():
    ...
    result = short.recognize("это тест для метода", profiles)
    check("коротких слов распознаёт русский", result["language"] == "русский", str(result))
    ...
    check("частотных слов распознаёт русский", result_f["language"] == "русский", str(result_f))

    section("6. Расстояние Out-of-Place")
    distance = out_of_place_distance(["а", "б", "в", "г", "д"], ["а", "г", "б", "д", "в"])
    check("ОоП учитывает ранги, а не алфавит", distance == 6, f"distance={distance}")
    empty = out_of_place_distance([], ["а"])
    check("ОоП для пустого профиля", empty == float("inf"))
    ...
    check("нейросеть распознаёт русский", result_nn["language"] == "русский", str(result_nn))
