...
from core.shared.syntax.analyzer import SyntaxAnalyzer
...
class SyntaxManager:
    ...
    def analyze_document(self, doc_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for sent in sentences:
                db.query(Token).filter(Token.sentence_id == sent.id).delete(synchronize_session=False)
                db.query(SyntaxRelation).filter(SyntaxRelation.sentence_id == sent.id).delete(synchronize_session=False)
            db.query(Sentence).filter(Sentence.document_id == doc_id).delete(synchronize_session=False)
            db.commit()
            ...
            for sentence_data in analysis:
                ...
                for token_data in tokens_data:
                    token = Token(
                        ...
                        token_index=token_data.get('token_index', token_data.get('position', 0)),
    ...
    def get_relations(self, doc_id: str) -> list[dict[str, Any]] | None:
        ...
        try:
            ...
            all_relations = []
            for sent in sentences:
                relations = db.query(SyntaxRelation).filter(
    ...
    def update_token_role(self, doc_id: str, sentence_id: int, token_id: int, syntax_role: str, syntax_role_name: str) -> bool:
        db = self._get_db()
        try:
