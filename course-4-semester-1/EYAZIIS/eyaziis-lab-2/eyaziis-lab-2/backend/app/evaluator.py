...
def evaluate(model_key: str = PRIMARY_MODEL) -> dict:
    ...
    recalls = []
    for label in labels:
        tp = sum(1 for row in per_query if row["predicted"] == label and label in row["relevant"])
        fp = sum(1 for row in per_query if row["predicted"] == label and label not in row["relevant"])
        fn = sum(1 for row in per_query if row["predicted"] != label and label in row["relevant"])
