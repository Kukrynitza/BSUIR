...
export interface SemanticAnnotation {
  ...
  semantic_role_description: string;
  syntax_basis: string;
...
export interface AnalysisResult {
  /** Есть только у ответов по сохранённому документу */
  document_id?: string;
  analysis: Sentence[];
  statistics: {
...
export interface DirectSemanticSentence {
  sentence_index: number;
  sentence_text: string;
  annotations: Array<SemanticAnnotation & { token_text?: string }>;
  links: Array<{
    sentence_id: number;
    from_token_id: number;
    to_token_id: number;
...
export interface TreeAnalysisResult {
  ...
  semantic_analysis?: DirectSemanticSentence[];
  statistics: {
    total_sentences: number;
    total_tokens: number;
    pos_distribution: Record<string, number>;
...
export const api = {
  ...
  async exportSemanticJson(docId: string): Promise<Record<string, unknown>> {
    const res = await fetch(`${API_BASE}/documents/${docId}/semantic-export`);
    if (!res.ok) throw new Error('Failed to export semantic');
    return res.json();
  },

  async updateSemanticAnnotation(
    docId: string,
    annotationId: number,
    payload: { entity_category?: string; semantic_role?: string; concept_label?: string }
  ): Promise<{ message?: string; analysis: Sentence[] }> {
    const res = await fetch(`${API_BASE}/documents/${docId}/semantic/annotation/${annotationId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
    if (!res.ok) throw new Error('Failed to update semantic annotation');
    return res.json();
  },
};
