  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <p className="muted">
          При запуске система сама распознаёт 10 эталонных фраз из разметки оценки
          (файлы <code>eval_01.txt</code> … <code>eval_10.txt</code>). PDF, загруженные
          на странице «Распознавание», попадают сюда отдельно.
        </p>
        {documents.length === 0 ? (
            ...
            <tbody>
              {documents.map((doc) => (
                <tr key={doc.id}>
                  ...
                  <td>
                    <span className={`language-badge ${doc.detected_language === 'немецкий' ? 'lang-german' : 'lang-russian'}`}>
